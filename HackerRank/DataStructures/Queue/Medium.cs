...
namespace HackerRank.DataStructures.Queue
{
    public class Medium
    {
        ...
        public static int minimumMoves(string[] grid, int startX, int startY, int goalX, int goalY)
        {
            ...
            // these are not in the traditional x y fashion
            ...
            int[] udlr = new int[4]; // up down left right
            ...
            while (q.Count > 0)
            {
                ...
                if (walks.Count() > 0) return walks.First();
            }
        ...
        private static int walk(string[] grid, Queue<Point> q, HashSet<string> visited, Point current, int goalX, int goalY, int x, int y)
        {
            ...
            Point next = new Point(current.X + x, current.Y + y, current.Count);
